...
exports.universal = mapTable => (_from = [0], _to = 1) => {
    ...
    return lodash_1.orderBy(mapTable.map(line => {
        const size = lodash_1.max(from.map(f => Array.isArray(line[f])
            ? lodash_1.max(line[f].map(i => i.length))
            : line[f].length));
        let result = [
            new RegExp(from
                .map(f => Array.isArray(line[f])
