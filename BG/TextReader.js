  ...
  alldivs.each((i, item) => {
    ...
    switch (current) {
      ...
      case 'text': {
        const txts = $(item)
          .text()
          .match(/ТЕКСТЫ* (\d{1,2}(.(\d{1,2}))?)(–(\d{1,2})){0,1}/);

        text = {};
        if (txts[5]) {
          text.name = `${txts[3]}-${txts[5]}`;
          text.text = [parseInt(txts[3], 10), parseInt(txts[5], 10)];
        } else {
          text.text = [parseInt(txts[3], 10)];
          const [, name] = txts;
          text.name = name;
        }
        chapter.texts.push(text);
...
console.time('parse');
const giteRu = readBook('./RU', russianChapter);
console.timeEnd('parse');

fs.writeFileSync('RU.json', out(giteRu));
